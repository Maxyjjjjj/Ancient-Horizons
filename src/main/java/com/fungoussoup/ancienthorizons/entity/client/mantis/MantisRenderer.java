...
public class MantisRenderer extends MobRenderer<MantisEntity, MantisModel<MantisEntity>> {
    public MantisRenderer(EntityRendererProvider.Context context) {
        super(context, new MantisModel<>(context.bakeLayer(MantisModel.LAYER_LOCATION)), 1f);
    }

    @Override
    public void render(MantisEntity entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
        if (entity.isBaby()) {
    ...
    public ResourceLocation getTextureLocation(MantisEntity mantisEntity) {
        return ResourceLocation.fromNamespaceAndPath(AncientHorizons.MOD_ID, "textures/entity/mantis/mantis.png");
    }
}
