...
public class HoatzinEntity extends Animal implements SemiFlyer {
    ...
    public void tick() {
        ...
        // Recovery stamina when not flying
    ...
    private void handleFlightMechanics() {
        if (isFlying()) {
            // Consume stamina while flying
            int currentStamina = getFlightStamina();
            if (currentStamina > 0) {
                setFlightStamina(currentStamina - FLIGHT_STAMINA_COST);
            } else {
                // Out of stamina, stop flying
                stopFlying();
                flightCooldown = 100; // 5 second cooldown
            }

            // Apply flying movement
            if (!this.onGround()) {
                Vec3 motion = this.getDeltaMovement();
                // Reduce fall speed when flying
                if (motion.y < 0) {
    ...
    public boolean canFly() {
        return !isTired() && flightCooldown <= 0 && !this.isBaby();
    }
