...
public abstract class AbstractLargeAzhdarchidEntity extends Animal implements SemiFlyer, NeutralMob, PlayerRideable, Saddleable {
    ...
    static class AzhdarchidAirAttackGoal extends Goal {
        ...
        public AzhdarchidAirAttackGoal(AbstractLargeAzhdarchidEntity azhdarchid) {
            ...
            // The goal requires movement control, which will be handled by the MoveControl
        ...
        public boolean canUse() {
            ...
            // 1. Must be flying
            if (!this.azhdarchid.isFlying()) return false;

            // 2. Must have a target
            if (target == null) return false;

            // 3. Target must be within a certain distance range (e.g., 8-40 blocks)
            double distanceSq = this.azhdarchid.distanceToSqr(target);
            if (distanceSq < 64 || distanceSq > 1600) return false; // Between 8 and 40 blocks

            // 4. Random check to prevent constant attacks (1 in 50 chance every tick)
            return this.azhdarchid.random.nextInt(50) == 0;
        }

        @Override
        public boolean canContinueToUse() {
            // Continue while diving or if the target is still valid and close enough
            if (this.diveTimer > 0) return true;

            LivingEntity target = this.azhdarchid.getTarget();
            if (target == null || !target.isAlive()) return false;

            // Only continue if we are flying and the target is still within attack range
            double distanceSq = this.azhdarchid.distanceToSqr(target);
        ...
        public void start() {
            LivingEntity target = this.azhdarchid.getTarget();
            if (target != null) {
                // Set the dive target to a point slightly below the target's position
                this.diveTarget = target.position().subtract(0, 1.0, 0);
                this.diveTimer = DIVE_DURATION;
                this.azhdarchid.setAttacking(true);
                this.azhdarchid.setAnimation(AZHDARCHID_DIVE); // Set the dive animation
            }
        }
        ...
        public void tick() {
            ...
            if (this.diveTimer > 0) {
                // Phase 1: Dive to the target point
                Vec3 currentPos = this.azhdarchid.position();
                Vec3 direction = this.diveTarget.subtract(currentPos).normalize();

                // Increase vertical velocity for a steep dive
                double speed = this.azhdarchid.getAttributeValue(Attributes.FLYING_SPEED) * 2.5;

                // Set movement towards the dive target
                this.azhdarchid.getMovementControl().setWantedPosition(
                        this.diveTarget.x, this.diveTarget.y, this.diveTarget.z, speed
                );

                // Face the target (or movement direction)
                float yaw = (float) (Mth.atan2(direction.z, direction.x) * 180.0 / Math.PI) - 90.0F;
                float newYaw = Mth.approachDegrees(this.azhdarchid.getYRot(), yaw, 10.0F);
                this.azhdarchid.setYRot(newYaw);
                this.azhdarchid.setYHeadRot(newYaw);

                if (this.azhdarchid.distanceToSqr(target) < 4.0D) {
                    this.azhdarchid.doHurtTarget(target);
                    this.azhdarchid.setAttacking(false);
                    this.azhdarchid.setAnimation(AZHDARCHID_PECK_AIR); // Play hit animation
                    this.diveTimer = 0; // Stop the dive immediately after hitting
                }

                this.diveTimer--;
            } else {
                // Phase 2: Pull up and try to gain altitude
                this.azhdarchid.getMovementControl().setWantedPosition(
                        this.azhdarchid.getX(), this.azhdarchid.getY() + 10.0, this.azhdarchid.getZ(),
                        this.azhdarchid.getAttributeValue(Attributes.FLYING_SPEED) * 1.5
                );

                // End the goal once altitude is gained or dive is complete
                if (this.azhdarchid.getDeltaMovement().y > 0.4) {
                    this.stop();
                }
            }
        }
