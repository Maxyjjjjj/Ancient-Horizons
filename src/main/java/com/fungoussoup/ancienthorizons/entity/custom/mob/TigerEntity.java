...
public class TigerEntity extends TamableAnimal implements NeutralMob, VariantHolder<TigerVariant> {
    ...
    private void setupAnimationStates() {
        ...
        // Handle standing animation with proper bounds checking
        if (this.getAnimation() == AMBUSH) {
            this.clientSideStandAnimation = Math.min(this.clientSideStandAnimation + 0.1F, 6.0F);
        } else {
            this.clientSideStandAnimation = Math.max(this.clientSideStandAnimation - 0.1F, 0.0F);
        }
    ...
    public void travel(Vec3 travelVector) {
        ...
        if (this.isInWater()) {
            ...
            // Floating upward when submerged
            if (this.getEyeY() < this.getFluidHeight(FluidTags.WATER)) {
                this.setDeltaMovement(this.getDeltaMovement().add(0.0, 0.03, 0.0));
            }

            // Basic water travel logic (swim forward if moving)
    ...
    public static class TigerYawnGoal extends Goal {
        ...
        public boolean canUse() {
            return !tiger.isStealth()
                    && !tiger.isRunning()
                    && !tiger.isOrderedToSit()
                    && tiger.getTarget() == null
                    && !tiger.isYawning()
                    && !tiger.isInWater()
                    && tiger.getNavigation().isDone()
                    && !tiger.isSleeping()
                    && tiger.getRandom().nextFloat() < 0.001F; // Rare idle yawn chance
        }
        ...
        public void start() {
            ...
            yawnTicks = 60; // ~3 seconds at 20 ticks/sec
        }
        ...
        public void tick() {
            ...
            if (yawnTicks == 0 && tiger.isYawning()) {
                tiger.setYawning(false);
            }
        }
    }
    ...
    abstract static class AbstractRoarGoal extends Goal {
        protected final TigerEntity tiger;
        protected final float volume;
        protected final int minInterval;
        protected final int maxInterval;
        protected int cooldown;

        protected AbstractRoarGoal(TigerEntity tiger, float volume, int minInterval, int maxInterval) {
            this.tiger = tiger;
            this.volume = volume;
        ...
        protected void resetCooldown() {
            cooldown = Mth.nextInt(tiger.getRandom(), minInterval, maxInterval);
        }
        ...
        @Override
        public boolean canUse() {
            return !tiger.isBaby() && !tiger.isStealth() && !tiger.isRunning() && cooldown <= 0;
        }
    }
    ...
    private static class TigerSleepGoal extends Goal {
        private final TigerEntity tiger;
        ...
        public boolean canUse() {
            return !tiger.isStealth()
                    ...
                    && tiger.getNavigation().isDone()
                    && !tiger.isYawning()
                    && tiger.getRandom().nextFloat() < 0.001F; // Rare idle nap chance
        }

        @Override
        public void start() {
            sleepTime = 200 + tiger.getRandom().nextInt(200); // Sleep for 10–20 seconds
