...
public class SemiFlyingMoveControl extends MoveControl {
    ...
    /**
     * Adjusts movement for energy conservation
     */
    public void adjustForEnergy() {
        if (this.flyer.isTired() && this.flyer.isFlying()) {
            // Force gliding when tired
            Mob mob = (Mob) this.flyer;
            Vec3 movement = mob.getDeltaMovement();
            mob.setDeltaMovement(movement.scale(0.8));
        }
    }
