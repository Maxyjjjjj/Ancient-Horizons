...
public class SemiAquaticFindWater extends Goal {
    private final PathfinderMob creature;
    private BlockPos targetPos;
    private final int executionChance = 30;

    public SemiAquaticFindWater(PathfinderMob creature) {
    ...
    public boolean canUse() {
        if (this.creature.onGround() && !this.creature.level().getFluidState(this.creature.blockPosition()).is(FluidTags.WATER)) {
            if (this.creature instanceof SemiAquaticAnimal && ((SemiAquaticAnimal) this.creature).shouldEnterWater() && (this.creature.getTarget() != null || this.creature.getRandom().nextInt(executionChance) == 0)) {
    ...
    public boolean canContinueToUse() {
        if (this.creature instanceof SemiAquaticAnimal && !((SemiAquaticAnimal) this.creature).shouldEnterWater()) {
    ...
    public BlockPos generateTarget() {
        ...
        final int range = this.creature instanceof SemiAquaticAnimal ? ((SemiAquaticAnimal) this.creature).getWaterSearchRange() : 14;
        for(int i = 0; i < 15; i++) {
