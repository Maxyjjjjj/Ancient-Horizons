...
public class TroopManager {
    ...
    public static void tickAllTroops(int maxPerTick) {
        ...
        while (processed < maxPerTick) {
            if (troops.isEmpty()) break; // Check the dynamic state of the collection
            List<Troop> refreshedList = new ArrayList<>(troops.values()); // Refresh the list snapshot
            Troop t = refreshedList.get(roundRobinIndex % refreshedList.size());
            t.tick();
            roundRobinIndex = (roundRobinIndex + 1) % refreshedList.size();
            processed++;
        }
    }
