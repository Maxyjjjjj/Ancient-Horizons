...
public class SemiAquaticLeaveWater extends Goal {
    private final PathfinderMob creature;
    private BlockPos targetPos;
    private final int executionChance = 30;

    public SemiAquaticLeaveWater(PathfinderMob creature) {
    ...
    public boolean canUse() {
        if (this.creature.level().getFluidState(this.creature.blockPosition()).is(FluidTags.WATER) && (this.creature.getTarget() != null || this.creature.getRandom().nextInt(executionChance) == 0)) {
            if (this.creature instanceof SemiAquaticAnimal && ((SemiAquaticAnimal) this.creature).shouldLeaveWater()) {
    ...
    public BlockPos generateTarget() {
        ...
        while(vector3d != null && tries < 8) {
            ...
            if (waterDetected) {
                ...
                return AHBlockPos.fromVec3(vector3d);
            }
            tries++;
        }
        return null;
